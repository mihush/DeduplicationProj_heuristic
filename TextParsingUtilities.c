...
void move_files_to_output_array(Dir current_dir , File* files_array , File* output_files_array ,
    ...
    for(int f = 0 ; f < current_dir->num_of_files  ; f++){
        ...
        if(curr_file == NULL){
            printf("ERROR - File in move_files_to_output_array is NULL .....\n");
        }
...
void update_dir_values(FILE *files_output_result , Dir current_dir, int goal_depth,
    ...
    if(current_dir->num_of_subdirs == 0){
        if(current_dir->num_of_files != 0){//There are still some files in the directory
            if(current_depth <= (goal_depth - 1)){
                ...
                for(unsigned long f = 0 ; f < current_dir->num_of_files  ; f++) {
    ...
    if(current_depth < (goal_depth - 1)){ //we HAVEN'T Reached the desired depth
        ...
        for(unsigned long d = 0 ; d < current_dir->num_of_subdirs ; d++){
            ...
            output_dirs_array[*output_dirs_idx]->sn = new_sub_dir_sn;
        ...
        for(unsigned long j = 0 ; j < current_dir->num_of_subdirs ; j++){
            ...
            update_dir_values(files_output_result , dirs_array[(current_dir->dirs_array)[j]] , goal_depth,
                              ...
                              base_object_array, num_base_object,
                              output_files_array, output_files_idx,
                              output_dirs_array , output_dirs_idx, current_depth,
                              merged_file_ht_size , original_depth , max_mf_mempool_cnt ,
                              merged_file_mem_pool ,memory_pool);
        }
...
void calculate_depth_and_merge_files(FILE *files_output_result , Dir* roots_array, int num_roots,
                                     Dir* dirs_array, unsigned long num_dirs,
                                     File* files_array,  unsigned long num_files,
    ...
    for(int r = 0 ; r < num_roots ; r++){
        ...
        output_dirs_array[*output_dirs_idx]->parent_dir_sn = *output_dirs_idx;
        (*output_dirs_idx)++;
