...
void update_dir_values(FILE *files_output_result , Dir current_dir, unsigned long dir_sn, int goal_depth, Dir* dirs_array, unsigned long num_dirs,
    ...
    if(current_depth < (goal_depth - 1)){ //we HAVEN'T Reached the desired depth
        ...
        for(int d = 0 ; d < current_dir->num_of_subdirs ; d++){
            ...
            update_dir_values(files_output_result , output_dirs_array[(current_dir->dirs_array)[d]], output_dirs_array[(current_dir->dirs_array)[d]]->sn ,goal_depth, dirs_array, num_dirs,
        ...
        for(int j = 0 ; j < current_dir->num_of_subdirs ; j++){
            ...
            //For each directory - call update_dir_values
            update_dir_values(files_output_result , dirs_array[(current_dir->dirs_array)[j]], dirs_array[(current_dir->dirs_array)[j]]->sn ,goal_depth,
...
void calculate_depth_and_merge_files(FILE *files_output_result , Dir* roots_array, int num_roots, Dir* dirs_array, unsigned long num_dirs,
    ...
    for(int r = 0 ; r < num_roots ; r++){
        ...
        update_dir_values(files_output_result, roots_array[r], roots_array[r]->sn , goal_depth, dirs_array, num_dirs, files_array, num_files,
